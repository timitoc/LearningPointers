...
var parse_after = require('./after_watch_added_parser');
...
Debugger.prototype.init = function(){
	...
	if(self.flags.add_watch){
		...
		if(self.buffer_stdout.startsWith("(gdb)")){
		    result = {
			expr: null,
			value: null 
		    }
		    self.socket.emit('debug', 'Variable not found');

		} else {
		    result=  parse_after(self.buffer_stdout);
		}
